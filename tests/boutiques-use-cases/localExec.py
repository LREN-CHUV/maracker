...
class LocalExecutor(object):
  ...
  """
    ...
    def __init__(self, desc, options={}):
        ...
        self.inputs = self.desc_dict[
            'inputs']  # Structure: [ {id:...},...,{id:...} ]
        # The set of output parameters from the json descriptor
        self.outputs = self.desc_dict[
            'output-files']  # Structure: [ {id:...},...,{id:...} ]
        # The set of parameter groups, according to the json descriptor
        self.groups = self.desc_dict[
            'groups'] if 'groups' in self.desc_dict.keys() else []
        ...
        self.launchDir = None if self.container is None else self.container.get(
            'working-directory')
        ## Extra Options ##
        ...
        if (not self.container is None) and self.container['type'] != 'docker':
            raise ValueError('Other container types than docker (e.g. ' + self.
                             container['type'] + ') are not yet supported')
    ...
    '''
        ...
        command, exit_code, container = self.cmdLine[
            0], None, self.container or {}
        print('Attempting execution of command:\n\t' + command +
              '\n---/* Start program output */---')
        # Check for docker
        dockerImage, dockerIndex = container.get('image'), container.get(
            'index')
        ...
        if 'environment-variables' in self.desc_dict.keys():
            for (envVarName, envVarValue) in [
                (p['name'], p['value'])
                    for p in self.desc_dict['environment-variables']
            ]:
                os.environ[envVarName], envVars[
                    envVarName] = envVarValue, envVarValue  # for non-container and docker resp.
        ...
        dsname = 'temp-' + str(int(time.time(
        ) * 1000)) + '.localExec.dockerjob.sh'  # time tag to avoid overwrites
        ...
        if dockerIsPresent and not self.ignoreContainer:
            ...
            userchange = '' if not self.changeUser else (
                "useradd --uid " + uid + ' ' + uname + "\n")
            ...
            launchDir = '${PWD}' if (self.launchDir is
                                     None) else self.launchDir
            # Run it in docker
            dcmd = 'docker run --rm' + envString + ' -v ${PWD}:' + launchDir + ' -w ' + launchDir + ' ' + str(
                dockerImage) + ' ./' + dsname
        ...
        for outfile in self.desc_dict['output-files']:
            ...
            isOptional = outfile[
                'optional'] if 'optional' in outfile.keys() else False
            s1 = 'Optional' if isOptional else 'Required'
            s2 = '' if exists else 'not '
            err = "Error! " if (
                not isOptional and not exists
            ) else ''  # Add error warning when required file is missing
    ...
    def _randomFillInDict(self):
        ...
        def randNum(p):
            ...
            isInt = self.safeGet(
                param_id,
                'integer')  # Check if the input parameter should be an int
            ...
            minv, maxv = self.safeGet(param_id, 'minimum'), self.safeGet(
                param_id, 'maximum')
        ...
        def makeParam(prm):
            ...
            return ' '.join(
                str(paramSingle(prm)) for _ in range(
                    rnd.randint(mn, mx))) if isList else paramSingle(prm)
        ...
        def mutReqs(targetParam):
            return map(
                lambda mutualReq: self.byId(mutualReq[0]),  # Get object corresponding to id
                filter(
                    lambda possibleMutReq: targetParam['id'] in possibleMutReq[1],  # Keep requirements that require the target
                    map(lambda reqOfTarg: (reqOfTarg, self.reqsOf(reqOfTarg)),
        ...
        def isOrCanBeFilled(targ):
            ...
            for r in self.reqsOf(targ['id']):
                if not r in self.in_dict:  # If a requirement is not present
                    if not targ['id'] in self.reqsOf(
                            r):  # and it is not mutually required
                        return False
            # If it is in a mutex group with one target already chosen, it cannot be filled
            g = self.assocGrp(
                targ['id'])  # Get the group that the target belongs to, if any
            if (not g is None) and self.safeGrpGet(g['id'],
                                                   'mutually-exclusive'):
                if len(
                        filter(lambda x: x in self.in_dict.keys(),
                               g['members'])) > 0:
                    return False
            return True
        ...
        for reqp in [
                r for r in self.inputs
                if self.safeGet(r['id'], 'optional') == False
        ]:
        ...
        for grp in [
            ...
            while True:  # Loop to choose an allowed value, in case a previous choice disabled that one
                choice = self.byId(
                    rnd.choice(grp['members']))  # Pick a random parameter
                res = checkMutualRequirements(
                    choice
                )  # see if it and its mutual requirements can be filled
                if res == False:
                    continue  # Try again if the chosen group member is not permissable
                for r in res:
    ...
    '''
        ...
        if stringInput:
            instrings = [
                s.strip().split(",") for s in infile.split(";") if not s == ''
            ]
            self.in_dict = {
                v[0].strip(): v[1].strip()
                for v in instrings if len(v) == 2
            }
        # File case
        else:
            ...
            with open(infile, 'r') as inparams:
                if infile.endswith('.csv'):  # csv case
                    lines = [
                        line.strip().split(",")
                        for line in inparams.readlines()
                    ]
                    self.in_dict = {
        ...
        for inprm in self.in_dict:
            if str(self.in_dict[inprm]).lower() == 'false' and self.byId(
                    inprm)['type'] == 'Flag':
                toRm.append(inprm)
            elif self.byId(
                    inprm)['type'] == 'Flag' and self.in_dict[inprm] == True:
                self.in_dict[
                    inprm] = "true"  # Fix json inputs using bools instead of strings
        for r in toRm:
            del self.in_dict[r]
        # Add default values for required parameters, if no value has been given
        for input in [
                s for s in self.inputs
                if not s.get("default-value") is None and not s.get("optional")
        ]:
            if self.in_dict.get(input['id']) is None:
                df = input.get("default-value")
                self.in_dict[input[
                    'id']] = df if not input['type'] == 'Flag' else str(df)
        # Check results (as much as possible)
        try:
            self._validateDict()
        except Exception:  # Avoid catching BaseExceptions like SystemExit
    ...
    def _replaceKeysInTemplate(self,
                               template,
                               useFlags=False,
                               unfoundKeys="remove",
                               strippedExtensions=[]):
        ...
        for paramId in map(lambda x: x['id'], self.inputs + self.outputs):
            ...
            if paramId in in_out_dict.keys():  # param has a value
                ...
                if useFlags:
                    ...
                    if self.safeGet(
                            paramId, 'type'
                    ) == 'Flag':  # special case for flag-type inputs
    ...
    def _generateOutputFileNames(self):
        ...
        for outputId in map(lambda x: x['id'], self.outputs):
            ...
            outputFileName = self.out_dict[
                outputId] if outputId in self.out_dict.keys(
            ...
            outputFileName = self._replaceKeysInTemplate(
                outputFileName, False, "keep", strippedExtensions)
    ...
    def _writeConfigurationFiles(self):
        for outputId in map(lambda x: x['id'], self.outputs):
            ...
            for line in fileTemplate:
                newTemplate.append(
                    self._replaceKeysInTemplate(line, False, "clear",
                                                strippedExtensions))
            template = "\n".join(newTemplate)
    ...
    def _generateCmdLineFromInDict(self):
        ...
        self._generateOutputFileNames(
        )  # it is required to call the method twice in case path templates contain output keys
    ...
    def _validateDict(self):
        ...
        for key in self.in_dict:
            ...
            def check(keyname, isGood, msg, value):  # Checks input values
                ...
                dontCheck = ((not keyname in targ.keys()) or
                             (targ[keyname] == False))
            ...
            if targ["type"] == "Number":
                ...
                check(None, lambda x, y: x.lower() in ["true", "false"],
                ...
                if not self.forcePathType:
                    ...
                    replacementFiles = []
                    launchDir = self.launchDir if (
                        not self.launchDir is None) else os.getcwd()
                    for ftarg in (str(val).split() if isList else [val]):
                        ...
                        if targ.get('uses-absolute-path') == True and (
                                not self.launchDir is None) and (
                                    not self.ignoreContainer):
                            relpath = os.path.relpath(ftarg, os.getcwd(
                            ))  # relative path to target, from the pwd
                            mountedAbsPath = os.path.join(
                                launchDir,
                                relpath)  # absolute path in the container
                        ...
                        elif targ.get('uses-absolute-path') == True:
        ...
        for givenVal in [
                v for v in self.inputs if v['id'] in self.in_dict.keys()
        ]:
            for r in self.reqsOf(
                    givenVal['id']):  # Check that requirements are present
            ...
            for d in (givenVal['disables-inputs']
                      if 'disables-inputs' in givenVal.keys() else []):
                if d in self.in_dict.keys():  # Check if a disabler is present
                    self.errs.append('Input ' + str(
                        d) + ' should be disabled by ' + str(givenVal['id']))
        ...
        for group, mbs in map(lambda x: (x, x["members"]), self.groups):
            ...
            if ("mutually-exclusive" in group.keys()
                ) and group["mutually-exclusive"]:
                if len(set.intersection(set(mbs), set(
                        self.in_dict.keys()))) > 1:
            ...
            if ("one-is-required" in group.keys()
                ) and group["one-is-required"]:
                if len(set.intersection(set(mbs), set(
                        self.in_dict.keys()))) < 1:
                    self.errs.append('Group ' + str(group["id"]) +
...
'''
    ...
    parser.add_argument(
        ...
        metavar='descriptor',
        nargs=1,
        help='The input JSON tool descriptor to test.')
    parser.add_argument(
        '-i',
        '--input',
        help=
        'Input parameter values with which to test the generation (.json or .csv).'
    )
    parser.add_argument(
        '-e',
        '--execute',
        action='store_true',
        help='Execute the program with the given inputs.')
    parser.add_argument(
        '-r',
        '--random',
        action='store_true',
        help='Generate a random set of input parameters to check.')
    parser.add_argument(
        '-n',
        '--num',
        type=int,
        help='Number of random parameter sets to examine.')
    parser.add_argument(
        '-s',
        '--string',
        help=
        "Take as input a semicolon-separated string of comma-separated tuples on the command line."
    )
    parser.add_argument(
        '--dontForcePathType',
        action='store_true',
        help=
        'Fail if an input does not conform to absolute-path specification (rather than converting the path type).'
    )
    parser.add_argument(
        '--changeUser',
        action='store_true',
        help=
        'Changes user in a container to the current user (prevents files generated from being owned by root)'
    )
    parser.add_argument(
        '--ignoreContainer',
        action='store_true',
        help='Attempt execution locally, even if a container is specified.')
    parser.add_argument(
        '-d',
        '--destroyTempScripts',
        action='store_true',
        help=
        'Destroys any temporary scripts used to execute commands in containers.'
    )
    ...
    if given(args.num) and args.num < 1:
        ...
        errExit(
            'The input file ' + str(args.input) + ' does not seem to exist',
            False)
    elif given(args.input) and not (args.input.endswith(".json") or
                                    args.input.endswith(".csv")):
    ...
    elif not args.random and (not given(args.input) and
                              not given(args.string)):
    ...
    # Prepare inputs
    inData = args.input if given(args.input) else (args.string
                                                   if given(args.string) else
                                                   None)

    # Generate object that will perform the commands
    executor = LocalExecutor(desc, {
        'forcePathType': not args.dontForcePathType,
